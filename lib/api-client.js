...
function normalizeConstructorItem(item) {
  ...
  return {
    ...
    url: productUrl(d.url, sku),
    raw: item,
  };
}

/**
 * Build an absolute woolworths.co.za product URL. The search payload carries a
 * ready product path in `data.url`; when it's absent, `prod/_/A-<id>` resolves
 * to the same product. Both forms return HTTP 200.
 */
function productUrl(path, sku) {
  const BASE = 'https://www.woolworths.co.za';
  if (path) return path.startsWith('http') ? path : `${BASE}/${String(path).replace(/^\//, '')}`;
  return sku ? `${BASE}/prod/_/A-${sku}` : null;
}
